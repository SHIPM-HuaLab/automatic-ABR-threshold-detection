function [lagIdx,res,lag,corrcoefm] = cross_test_3signal(signals)
%CROSS_TEST_3SIGNAL calculate 3 averaged ABR signals and 2-by-2 xcorr
...
lagIdx = find(abs(lag-size(signals,2))<=round(size(signals,2)/100));
res = length(lagIdx);
